...
for pp = 1:length(noise_scale_factors)
    ...
    for p = 10:length(gsyn_scale_factors)
        gsyn_scaler = gsyn_scale_factors(p);

        for i = 1:nreps   
            
            [data, data_AP, APfreq, current,spiketrain, gsyn] = neuron_model(0,noise_scaler,gsyn_scaler);
            ...
            data_store{i,5} = energy_calc(current,data_store{i,3}); % calculate ATP used on current and APs
