...
%     nreps=1
%     binned_data = reshape(binned_data,[1,(500*334)])
%     %% reads 'words' over all response repetitions
    for x = 1:nreps
       for i = 1:length(binned_data(1,:))-(wordlength-1)
           responsewords{x,i} = num2str(binned_data(x,i:i+wordlength-1));
       end
    end
    ...
    for x = 1:nreps % loop through all repetitions
         ...
         words = responsewords(x,:);
         response_probs = sort(countmember(unique(words),words),'descend')/size(responsewords,2); % vector of the word probabilities present in the column

         for i = 1:length(response_probs)        
             H(i) = response_probs(i)*log2(response_probs(i));
         end
         
         clear response_probs
    ...
    for i = 1:size(responsewords,2)

        noise_column = responsewords(:,i);
        column_words = unique(noise_column)'; % finds unique 'words' in the column
        column_words_freq = countmember(unique(column_words),noise_column); % counts unique 'words'
