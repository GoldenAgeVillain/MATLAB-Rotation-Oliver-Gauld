function [tspan1,data, data_AP, APfreq, iInj_plot, ind] = simple_neuron(switchPlot);
...
SynCond(SynCond < 0)=0;% [mS]
SynCond = (SynCond/10) * scalefactor;
...
%% main nested subfunction
    function ds = fxn(t,s)
        V = s(1);
        iInj    = -SynCond(floor(t/dt)+1)*(V-EAMPA); % [nA]
